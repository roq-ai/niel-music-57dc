...
import { getPlaylistMusicById } from 'apiSdk/playlist-musics';
...
function PlaylistMusicViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Playlist Music Detail View
          </Text>
          {hasAccess('playlist_music', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/playlist-musics/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              ...
              {hasAccess('music', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/music/view/${data?.music?.id}`}>
                      {data?.music?.title}
                    </Link>
                  </Text>
                </Flex>
