...
import { getMusic, deleteMusicById } from 'apiSdk/music';
...
function MusicListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<MusicInterface[]>(
    ...
    () =>
      getMusic({
        relations: ['company', 'playlist_music.count'],
      }),
  );
  ...
  const handleView = (id: string) => {
    if (hasAccess('music', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/music/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('music', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/music/create`} passHref legacyBehavior>
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('playlist_music', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.playlist_music}</Td>
                    )}
                    <Td>
                      {hasAccess('music', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/music/edit/${record.id}`} passHref legacyBehavior>
                          <Button
                      ...
                      {hasAccess('music', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                        <IconButton
                          onClick={(e) => {
                            e.stopPropagation();
